...
class TaskSearcher:
    ...
    def search(
        ...
        pattern = (
            f"{path_pattern}/*.md"
            if path_pattern and not path_pattern.endswith(".md")
            else "**/*.md"
        )
        results = []
    ...
    def search_by_tags(
        self, tags: List[str], match_all: bool = False, max_results: int = 50
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _matches_metadata(
        self, metadata: Dict[str, Any], filters: Dict[str, Any], match_all: bool = False
    ) -> bool:
